...
def generate_pdf_summary(
    results: Dict,
    inputs: Dict,
    company_name: str = "LBO Deal Screener"
) -> Optional[bytes]:
    """
    ...
    comparison_data = [
        ...
        ['Enterprise Value', f"${entry_ev/1_000_000:.1f}M", f"${exit_ev/1_000_000:.1f}M", f"{ev_change:+.1f}%"],
        ['EBITDA', f"${entry_ebitda/1_000_000:.1f}M", f"${exit_ebitda/1_000_000:.1f}M", f"{ebitda_change:+.1f}%"],
    ]

    comparison_table = Table(comparison_data, colWidths=[2*inch, 1.5*inch, 1.5*inch, 1*inch])
    ...
    elements.append(Paragraph(f"Report generated on {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}", footer_style))
