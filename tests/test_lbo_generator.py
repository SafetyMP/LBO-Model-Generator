...
def test_balance_sheet_balancing():
    ...
    test_config = {
        ...
        "revenue_growth_rate": test_config.get("revenue_growth_rate", [0.15, 0.12, 0.10, 0.08, 0.07]),
    ...
    try:
        ...
        for year in model.years:
            ...
            if diff > 1.0:  # Allow small rounding differences
                print(f"  ⚠️  Year {year}: Assets = ${assets:,.2f}, Liab+EQ = ${liab_eq:,.2f}, Diff = ${diff:,.2f}")
                all_balance = False
        
        if all_balance:
            print("✓ Balance sheets balance for all years")
            return True
        else:
            print("✗ Some balance sheets do not balance")
            return False
...
def test_cash_flow_reconciliation():
    ...
    try:
        ...
        for i, year in enumerate(model.years):
            ...
            if diff > 0.01:
                print(f"  ⚠️  Year {year}: Previous ending (${prev_ending:,.2f}) != Current beginning (${curr_beginning:,.2f})")
                all_reconcile = False
            ...
            if abs(calc_ending - ending) > 0.01:
                print(f"  ⚠️  Year {year}: Beginning + Net Change != Ending")
                all_reconcile = False
        
        if all_reconcile:
            print("✓ Cash flow reconciles properly")
            return True
        else:
            print("✗ Cash flow reconciliation issues")
            return False
...
def test_debt_schedule():
    ...
    try:
        ...
        if len(test_config['debt_instruments']) > 1:
            ...
            print("✓ Debt schedule calculations correct")
...
def test_ai_recommendations():
    ...
    try:
        ...
        if not api_key:
            print("⚠️  Skipping AI test: OPENAI_API_KEY not set")
        ...
        print(f"🔄 Getting AI recommendations for {company_name}...")
        ...
        assert len(recommendations.get('debt_instruments', [])) > 0, "Should have debt recommendations"
        ...
        print("\n🔄 Testing model generation from AI recommendations...")
        ...
        print("⚠️  Skipping AI test: openai package not installed")
