...
class IndustryStandardExcelExporter:
    ...
    def _calculate_uses(self, total_debt: float) -> Tuple[List[Tuple[str, float]], float]:
        ...
        total_uses = (
            self.model.total_uses
            if hasattr(self.model, "total_uses")
            else (
        ...
        uses = [
            ...
            (
                ...
                (
                    self.model.financing_fees
                    if hasattr(self.model, "financing_fees")
    ...
    def _create_sources_uses_sheet(self):
        ...
        ws.merge_cells("A1:D1")
        IndustryStandardTemplate.format_header_cell(ws["A1"], "SOURCES & USES OF FUNDS", "A1:D1")
    ...
    def _create_assumptions_sheet(self):
        ...
        IndustryStandardTemplate.format_header_cell(ws["A1"], "MODEL ASSUMPTIONS", "A1:B1")
        ...
        for year in self.model.years:
            try:
                ...
                if "Depreciation" in self.model.income_statement.index:
                    dep = (
                        self.model.income_statement.loc["Depreciation", year]
                        if pd.notna(self.model.income_statement.loc["Depreciation", year])
                        else 0
                    )
                if "Amortization" in self.model.income_statement.index:
                    amort = (
                        self.model.income_statement.loc["Amortization", year]
                        if pd.notna(self.model.income_statement.loc["Amortization", year])
                        else 0
                    )
                ...
                if abs((revenue - cogs) - gross_profit) > 0.01:
                    validation_issues.append(
                        (
                            year,
                            f"Gross Profit: Revenue (${revenue:,.0f}) - COGS (${cogs:,.0f}) ≠ Gross Profit (${gross_profit:,.0f})",
                        )
                    )
                ...
                if abs((gross_profit - sganda) - ebitda) > 100:
                    validation_issues.append(
                        (
                            year,
                            f"EBITDA: Gross Profit (${gross_profit:,.0f}) - SG&A (${sganda:,.0f}) ≠ EBITDA (${ebitda:,.0f})",
                        )
                    )
                ...
                if abs((pretax - tax) - net_income) > 0.01:
                    validation_issues.append(
                        (
                            year,
                            f"Net Income: Pretax (${pretax:,.0f}) - Tax (${tax:,.0f}) ≠ Net Income (${net_income:,.0f})",
                        )
                    )
    ...
    def _add_cross_sheet_consistency_check(self, ws: openpyxl.worksheet.worksheet.Worksheet, row: int) -> int:
        ...
        if is_net_income_row and cf_net_income_row:
            row = self._add_consistency_formula_row(
                ws,
                "Net Income Check (IS vs CF):",
    ...
    def _add_consistency_formula_row(
        self,
        ws: openpyxl.worksheet.worksheet.Worksheet,
        label: str,
        sheet1_name: str,
    ...
    def _create_cash_flow_sheet(self):
        ...
        for label, key in cf_items:
            ...
            for year_idx, year in enumerate(self.model.years, start=6):
                try:
                    value = (
                        self.model.income_statement.loc["Net Income", year]
                        if key == "Net Income"
    ...
    def _format_base_metric_cell(
        self, ws: openpyxl.worksheet.worksheet.Worksheet, label: str, value: Any, row: int
    ) -> None:
    ...
    def _create_sheet_header(
        ...
        """
        ws.merge_cells(merge_range)
        IndustryStandardTemplate.format_header_cell(ws.cell(row=row, column=1), title, merge_range)
        return row + 1

    def _add_section_header(
        self, ws: openpyxl.worksheet.worksheet.Worksheet, title: str, row: int, col: int = 1
    ) -> int:
    ...
    def _add_financial_statement_rows(
        self,
        ws: openpyxl.worksheet.worksheet.Worksheet,
            ...
            for issue in validation_issues:
                if len(issue) == 2:
                    ...
                    ws.cell(row=row, column=1).value = (
                        f"  {name} Year {year}: Beginning - Principal ≠ Ending (diff: ${diff:,.0f})"
                    )
                ws.cell(row=row, column=1).font = IndustryStandardTemplate.FONT_BODY
                row += 1
    ...
    def _create_capital_structure_charts(self, ws_bs: openpyxl.worksheet.worksheet.Worksheet) -> None:
        ...
        try:
            ...
            try:
                ...
                if len(chart.series) >= 1:
                    self._configure_chart_series(
                        chart.series[0], debt_title_ref, marker_symbol="square", show_labels=False
                    )
    ...
    def _find_sensitivity_table_location(
        self, ws_sens: openpyxl.worksheet.worksheet.Worksheet
    ...
    def _create_moic_sensitivity_chart(
        self,
    ...
    def _create_irr_sensitivity_chart(
        self,
        ws_sens: openpyxl.worksheet.worksheet.Worksheet,
        categories: Reference,
        irr_data: Reference,
    ...
    def _create_combined_sensitivity_chart(
        self,
        ws_sens: openpyxl.worksheet.worksheet.Worksheet,
        categories: Reference,
        moic_data: Reference,
    ...
    def _add_charts(self) -> None:
        ...
        try:
            ...
            _ = ChartStructureImprover()  # Initialize but don't store
        except ImportError:
