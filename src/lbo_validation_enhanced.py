...
class EnhancedLBOValidator:
    ...
    def _validate_entry_metrics(config: Dict) -> tuple[List[str], List[str]]:
        ...
        elif not EnhancedLBOValidator.RANGES["entry_ebitda"][0] <= ebitda <= EnhancedLBOValidator.RANGES["entry_ebitda"][1]:
        ...
        elif not EnhancedLBOValidator.RANGES["entry_multiple"][0] <= entry_mult <= EnhancedLBOValidator.RANGES["entry_multiple"][1]:
    ...
    def _validate_debt_structure(config: Dict, ebitda: float) -> tuple[List[str], List[str]]:
        ...
        for i, debt in enumerate(debt_instruments):
            if "interest_rate" in debt:
                ...
                if not EnhancedLBOValidator.RANGES["interest_rate"][0] <= ir <= EnhancedLBOValidator.RANGES["interest_rate"][1]:
    ...
    def validate_debt_structure(config: Dict) -> Tuple[bool, List[str]]:
        ...
        interest_rates = [(d.get("interest_rate", 0), d.get("priority", 999)) 
                          for d in debt_instruments if "interest_rate" in d]
