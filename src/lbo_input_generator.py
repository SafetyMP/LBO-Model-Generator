        ...
        debt_instruments.append(
            {
                ...
                "amount": (
                    float(input("  Fixed Amount ($, if not using EBITDA multiple): ") or "0")
...
def _collect_operating_assumptions(config: Dict) -> None:
    ...
    config["sganda_pct_of_revenue"] = (
        float(input(f"SG&A as % of Revenue [{default_sganda:.1f}%]: ") or default_sganda) / 100
    )

    default_capex = (config.get("capex_pct_of_revenue") or 0.03) * 100
    config["capex_pct_of_revenue"] = (
        float(input(f"CapEx as % of Revenue [{default_capex:.1f}%]: ") or default_capex) / 100
    )
...
def _collect_working_capital(config: Dict) -> None:
    ...
    config["days_inventory_outstanding"] = float(
        input(f"Days Inventory Outstanding [{default_dio:.0f}]: ") or default_dio
    )
...
def _run_ai_validation(model: "LBOModel", args) -> None:
    ...
    validation_result = model.validate_with_ai(
        industry=getattr(args, "industry", None), api_key=getattr(args, "api_key", None)
    )
...
def _run_ai_review(model: "LBOModel", output_file: str, args) -> None:
    ...
    review_result = model.review_output_with_ai(output_file, api_key=getattr(args, "api_key", None))
