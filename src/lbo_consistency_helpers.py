...
class LBOConsistencyHelper:
    ...
    def ensure_consistent_structure(config: Dict, include_sub_debt: bool = False) -> Dict:
        ...
        if "debt_instruments" in standardized:
            has_senior = any("Senior" in d.get("name", "") or "senior" in d.get("name", "").lower() 
                           for d in standardized["debt_instruments"])
            has_sub = any("Sub" in d.get("name", "") or "sub" in d.get("name", "").lower() 
                         for d in standardized["debt_instruments"])
            ...
            if include_sub_debt and not has_sub:
                ...
                senior_debt = next((d for d in standardized["debt_instruments"] 
                                   if "Senior" in d.get("name", "")), None)
                if senior_debt:
                    sub_debt = {
    ...
    @staticmethod
    def standardize_all_configs(config_files: Dict[str, str], 
        ...
        for case_name, file_path in config_files.items():
            ...
            standardized_config = LBOConsistencyHelper.ensure_consistent_structure(
                standardized_config, 
                include_sub_debt=False  # Don't force sub debt
            )
