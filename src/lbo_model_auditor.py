...
class LBOModelAuditor:
    ...
    def audit_model_consistency(self, test_cases: Dict[str, Dict]) -> AuditReport:
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert LBO model auditor specializing in financial modeling consistency and best practices."},
    ...
    def audit_test_case_alignment(self, case_name: str, config: Dict, 
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert LBO model auditor specializing in test case validation and target metric alignment."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=1500
    ...
    def audit_chart_structure(self, excel_file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert in financial model visualization and Excel chart design for LBO models. Always return valid JSON only, no markdown formatting."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=2000
...
"""
    ...
    def _create_test_case_audit_prompt(self, case_name: str, config: Dict, 
                                       target_metrics: Optional[Dict]) -> str:
