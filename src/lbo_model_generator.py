...
class LBOAssumptions:
    ...
    def __post_init__(self):
        ...
        if expected_years < 1:
            ...
            errors.append(f"revenue_growth_rate has {expected_years} years, which exceeds typical LBO projection period (typically 5-7 years)")
...
class ExcelRowFinder:
    ...
    @staticmethod
    def find_row_by_label(ws: openpyxl.worksheet.worksheet.Worksheet, 
                         label: str, 
                         start_row: int = 1, 
                         end_row: int = None,
                         column: int = 1,
                         partial_match: bool = False) -> Optional[int]:
    ...
    @staticmethod
    def build_row_map(ws: openpyxl.worksheet.worksheet.Worksheet,
                     start_row: int = 1,
                     end_row: int = None,
                     column: int = 1) -> Dict[str, int]:
    ...
    @staticmethod
    def find_rows_by_patterns(ws: openpyxl.worksheet.worksheet.Worksheet,
                              patterns: Dict[str, List[str]],
                              start_row: int = 1,
                              end_row: int = None,
                              column: int = 1) -> Dict[str, Optional[int]]:
...
class LBOModel:
    ...
    def _calculate_transaction_values(self) -> None:
        ...
        total_uses_calc = self._round_value(self.equity_value + self.assumptions.existing_debt + self.transaction_expenses + self.financing_fees)
        ...
        self.total_uses = self._round_value(self.equity_value + self.assumptions.existing_debt + self.transaction_expenses + self.financing_fees)
        ...
        net_book_value = (self.assumptions.initial_ppe + 
        ...
        if self.goodwill < 0:
            logger.debug(f"Negative goodwill calculated: ${self.goodwill:,.0f}. This may indicate net book value exceeds purchase price.")
    ...
    def _build_balance_sheet(self) -> None:
        ...
        initial_ar = self.assumptions.initial_ar if self.assumptions.initial_ar > 0 else (
            self.assumptions.starting_revenue * self.assumptions.days_sales_outstanding / 365
        )
        initial_inv = self.assumptions.initial_inventory if self.assumptions.initial_inventory > 0 else (
            self.assumptions.starting_revenue * self.assumptions.cogs_pct_of_revenue * 
            self.assumptions.days_inventory_outstanding / 365
        )
        initial_ap = self.assumptions.initial_ap if self.assumptions.initial_ap > 0 else (
            self.assumptions.starting_revenue * self.assumptions.cogs_pct_of_revenue * 
            self.assumptions.days_payable_outstanding / 365
        )
        initial_ppe = self.assumptions.initial_ppe if self.assumptions.initial_ppe > 0 else (
            self.assumptions.starting_revenue * LBOConstants.DEFAULT_PPE_TO_REVENUE_RATIO
        )
        ...
        for year in self.years:
            ...
            self.balance_sheet.loc['Inventory', year] = self._round_value(
                revenue * self.assumptions.cogs_pct_of_revenue * 
                self.assumptions.days_inventory_outstanding / 365
            )
    ...
    def _calculate_working_capital_changes(self, year: int) -> float:
        ...
        if year == 1:
            prev_ar = self.balance_sheet.loc['Accounts Receivable', 1] if pd.notna(self.balance_sheet.loc['Accounts Receivable', 1]) else 0
            prev_inv = self.balance_sheet.loc['Inventory', 1] if pd.notna(self.balance_sheet.loc['Inventory', 1]) else 0
            prev_ap = self.balance_sheet.loc['Accounts Payable', 1] if pd.notna(self.balance_sheet.loc['Accounts Payable', 1]) else 0
        else:
            prev_ar = self.balance_sheet.loc['Accounts Receivable', year - 1] if pd.notna(self.balance_sheet.loc['Accounts Receivable', year - 1]) else 0
            prev_inv = self.balance_sheet.loc['Inventory', year - 1] if pd.notna(self.balance_sheet.loc['Inventory', year - 1]) else 0
            prev_ap = self.balance_sheet.loc['Accounts Payable', year - 1] if pd.notna(self.balance_sheet.loc['Accounts Payable', year - 1]) else 0
        
        curr_ar = self.balance_sheet.loc['Accounts Receivable', year] if pd.notna(self.balance_sheet.loc['Accounts Receivable', year]) else 0
        curr_inv = self.balance_sheet.loc['Inventory', year] if pd.notna(self.balance_sheet.loc['Inventory', year]) else 0
        curr_ap = self.balance_sheet.loc['Accounts Payable', year] if pd.notna(self.balance_sheet.loc['Accounts Payable', year]) else 0
    ...
    def _calculate_financing_activities_year1(self, total_debt_repayment: float) -> float:
        ...
        cff = self._round_value(
            debt_issuance + equity_contribution +
            purchase_price + existing_debt_repayment + transaction_expenses_cf + financing_fees_cf -
    ...
    def _reconcile_cash_balance(self, year: int, beginning_cash: float, 
                                cfo: float, capex: float, cff: float) -> float:
                ...
                if total_available_for_sweep > 0.01:  # Small tolerance for rounding
                    ...
                    for debt in sorted_debt:
                        ...
                        actual_sweep = self._apply_sweep_to_single_debt(
                            debt, year, year_idx, remaining_sweep, min_cash,
                            original_debt_schedule, is_exit_year
                        )
                    ...
                    self.balance_sheet.loc['Total Liabilities', year] = self._round_value(total_current_liab + total_debt)
    ...
    def _calculate_available_cash_for_sweep(self, year: int, year_idx: int, min_cash: float, 
                                            original_debt_schedule: Dict) -> float:
    ...
    def _update_debt_schedule_after_sweep(self, debt: LBODebtStructure, year_idx: int,
                                          actual_sweep: float) -> None:
    ...
    def _update_cash_flow_after_sweep(self, debt: LBODebtStructure, year: int, year_idx: int,
                                      actual_sweep: float, min_cash: float) -> float:
    ...
    def _apply_sweep_to_single_debt(self, debt: LBODebtStructure, year: int, year_idx: int,
                                     sweep_amount: float, min_cash: float, 
                                     original_debt_schedule: Dict, is_exit_year: bool) -> float:
    ...
    def _recalculate_financial_statements_after_sweep(self, future_year: int, future_year_idx: int,
                                                      original_debt_schedule: Dict, iteration: int) -> None:
        ...
        for debt in self.assumptions.debt_instruments:
            ...
            if beg_balance <= 0.01:
                ...
                if already_swept <= 0.01:
                    ...
                    if debt.amortization_schedule == "amortizing":
                        years_amortized = sum(1 for i in range(future_year_idx) 
                    ...
                    self.debt_schedule[debt.name]['principal_paid'][future_year_idx] = self._round_value(scheduled_principal)
    ...
    def _validate_debt_basic_checks(self, debt_name: str, year: int, year_idx: int,
                                    beg_bal: float, principal: float, interest: float,
                                    end_bal: float, debt: LBODebtStructure,
        ...
        if end_bal < -tolerance:
            errors.append(
                f"{debt_name} Year {year}: Ending balance (${end_bal:,.2f}) is negative"
            )
    ...
    def _validate_amortizing_schedule(self, debt_name: str, year: int, year_idx: int,
                                      principal: float, end_bal: float, debt: LBODebtStructure,
                                      tolerance: float, scenarios: Dict) -> List[str]:
    ...
    def _validate_bullet_schedule(self, debt_name: str, year: int, year_idx: int,
                                 principal: float, beg_bal: float, has_sweep: bool,
                                 tolerance: float, scenarios: Dict) -> List[str]:
    ...
    def _validate_final_year_debt(self, debt_name: str, final_end_bal: float,
                                  tolerance: float) -> List[str]:
    ...
    def _track_debt_scenarios(self, debt: LBODebtStructure, debt_name: str,
                              schedule: Dict, is_amortizing: bool, is_bullet: bool,
                              has_sweep: bool, tolerance: float, scenarios: Dict) -> None:
        ...
        if has_sweep:
            total_sweep = sum(
                max(0, schedule['principal_paid'][i] - 
    ...
    def _identify_mixed_structure(self, scenarios: Dict) -> None:
        ...
        amortizing_count = len([d for d in self.assumptions.debt_instruments 
                               if d.amortization_schedule == "amortizing"])
        bullet_count = len([d for d in self.assumptions.debt_instruments 
                           if d.amortization_schedule == "bullet"])
    ...
    def _validate_debt_schedule(self) -> Dict[str, List[str]]:
        ...
        for debt in self.assumptions.debt_instruments:
            ...
            for year_idx, year in enumerate(self.years):
                ...
                errors.extend(self._validate_debt_basic_checks(
                    debt_name, year, year_idx, beg_bal, principal, interest,
                    end_bal, debt, tolerance
                ))
                ...
                if is_amortizing:
                    warnings.extend(self._validate_amortizing_schedule(
                        debt_name, year, year_idx, principal, end_bal, debt, tolerance, scenarios
                    ))
                elif is_bullet:
                    errors.extend(self._validate_bullet_schedule(
            ...
            self._track_debt_scenarios(debt, debt_name, schedule, is_amortizing, 
                                       is_bullet, has_sweep, tolerance, scenarios)
    ...
    def _reconcile_balance_sheet(self, year: int) -> None:
        ...
        if diff > LBOConstants.BALANCE_SHEET_TOLERANCE:
            ...
            self.balance_sheet.loc['Shareholders Equity', year] = equity
            self.balance_sheet.loc['Total Liabilities & Equity', year] = self._round_value(total_liab + equity)
            logger.debug(f"Adjusted equity in year {year} to ${equity:,.2f} to balance sheet (Assets=${assets:,.2f}, Liab=${total_liab:,.2f})")
    ...
    def _validate_year1_sources_uses(self, year: int, ending_cash: float) -> None:
        ...
        expected_uses = (self.equity_value + self.assumptions.existing_debt + 
                        self.transaction_expenses + self.financing_fees)
        ...
        expected_ending_cash = (self.assumptions.existing_cash + net_transaction + 
                               cfo_year1 + capex_year1)
    ...
    def validate_with_ai(self, industry: Optional[str] = None, api_key: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            logger.info(f"AI validation completed: Valid={result.is_valid}, "
    ...
    def review_generated_model_ai(self, excel_file_path: str, api_key: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            logger.info(f"AI review completed: Valid={result.is_valid}, "
                       f"Warnings={len(result.warnings)}, Errors={len(result.errors)}")
    ...
    def generate_scenarios_with_ai(self, industry: Optional[str] = None, 
    ...
    def diagnose_error_ai(self, error_message: str, stack_trace: Optional[str] = None,
                         api_key: Optional[str] = None) -> Dict:
        """
    ...
    def calculate_returns(self) -> Dict:
        ...
        equity_invested = self.assumptions.equity_amount if self.assumptions.equity_amount > 0 else (
            self.equity_value - sum(d.amount for d in self.assumptions.debt_instruments)
        )
    ...
    def export_to_excel(self, filename: str, company_name: str = "LBO Model", 
                       interactive: bool = True,
                       validate_with_ai: bool = False, industry: Optional[str] = None,
