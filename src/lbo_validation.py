    ...
    try:
        ...
        if not path_obj.parent.exists():
            raise LBOConfigurationError(
                f"Output directory does not exist: {path_obj.parent}"
            )
        
        # Check if parent directory is writable
        if not os.access(path_obj.parent, os.W_OK):
            raise LBOConfigurationError(
                f"Output directory is not writable: {path_obj.parent}"
            )
...
def _validate_percentage_fields(config: Dict) -> List[str]:
    ...
    errors = []
    percentage_fields = [
        'cogs_pct_of_revenue', 'sganda_pct_of_revenue', 'capex_pct_of_revenue',
        'tax_rate', 'transaction_expenses_pct', 'financing_fees_pct'
    ]
